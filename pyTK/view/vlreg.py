...
class Lreg(Canvas):
    ...
    def addPoint(self,i=0,x=0,y=0):
        if self.find_withtag("myText%d"%i):
            ...
            self.controller.getText(i=i)
            self.create_text(x,self.height-y,text=str(self.controller.currentPoint),tags="myText%d"%i)
    ...
    def __init__(self,parent,graphData,width=256,height=192,
                 bg="white",stx=20,sty=20,controller=None):
        Canvas.__init__(self, parent)
        self.pack()
        self.graphData = graphData
        self.controller = controller
        self.controller.graphData = graphData
        self.width=width
        self.height=height
        self.bg = bg
        self.stx=stx
